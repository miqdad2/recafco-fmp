...
import { IssueSummaryCards } from './_components/issue-summary-cards';
import { IssueFilterBar } from './_components/issue-filter-bar';
import { IssueRegisterTable } from './_components/issue-register-table';
import { IssueActionsBar } from './_components/issue-actions-bar';
...
export default async function ContractIssuesPage({ searchParams }: PageProps): Promise<React.JSX.Element> {
  ...
  const raisedDateFrom = str(params['raisedDateFrom']);
  const raisedDateTo = str(params['raisedDateTo']);
  ...
  const hasActiveFilters = Boolean(
    search || contractId || status || priority || category || responsibleUserId || departmentId ||
    raisedDateFrom || raisedDateTo || dueDateFrom || dueDateTo || overdueOnly,
  );

  const [listRes, contractsRes, deptsRes, peopleRes, dashboardRes] = await Promise.allSettled([
    contractsApi.listIssues({
      page,
      pageSize: 25,
      ...(search ? { search } : {}),
      ...
      ...(raisedDateFrom ? { raisedDateFrom } : {}),
      ...(raisedDateTo ? { raisedDateTo } : {}),
  ...
  if (listRes.status === 'rejected') {
    error = listRes.reason instanceof Error ? listRes.reason.message : 'Failed to load issues';
  }
  ...
  function buildHref(overrides: Record<string, string | undefined>): string {
    ...
    const merged: Record<string, string | undefined> = {
      search, contractId, status, priority, category, responsibleUserId, departmentId,
      raisedDateFrom, raisedDateTo, dueDateFrom, dueDateTo,
    ...
    const s = q.toString();
    return s ? `/contracts/issues?${s}` : '/contracts/issues';
  }
  ...
  return (
    <div className="px-6 lg:px-8 py-6 max-w-[1920px] mx-auto space-y-6 print:px-0 print:py-0">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1.5 text-sm text-text-secondary">
            Track open issues, responsibilities, due dates and closure status across all contracts.
          </p>
        </div>
      ...
      <div className="hidden print:block">
        <h1 className="text-xl font-semibold">Contract Issue Log</h1>
        <p className="text-xs text-text-secondary">
          Generated {new Date().toLocaleString('en-GB')}
          {hasActiveFilters ? ' — filtered view' : ' — all issues'}
        </p>
      </div>

      <IssueSummaryCards summary={summary} />
      ...
      <IssueRegisterTable issues={issues} contracts={contracts} people={people} canUpdate={canUpdate} />

      {totalPages > 1 && (
        <div className="flex items-center justify-between text-sm text-text-secondary print:hidden">
          <span>Showing {issues.length} of {total}</span>
