...
import { PaymentRegisterTable } from './_components/payment-register-table';
import { PaymentActionsBar } from './_components/payment-actions-bar';

export const metadata: Metadata = { title: 'Contract Payments — RECAFCO FMP' };
...
export default async function ContractPaymentsPage({ searchParams }: PageProps): Promise<React.JSX.Element> {
  ...
  const invoiceDateFrom = str(params['invoiceDateFrom']);
  const invoiceDateTo = str(params['invoiceDateTo']);
  ...
  const hasActiveFilters = Boolean(
    search || contractId || company || status || departmentId || ownerUserId ||
    invoiceDateFrom || invoiceDateTo || dueDateFrom || dueDateTo || overdueOnly,
  );

  const [listRes, contractsRes, deptsRes, peopleRes, dashboardRes] = await Promise.allSettled([
    contractsApi.listPayments({
      page,
      pageSize: 25,
      ...(search ? { search } : {}),
  ...
  if (listRes.status === 'rejected') {
    error = listRes.reason instanceof Error ? listRes.reason.message : 'Failed to load payments';
  }
  ...
  function buildHref(overrides: Record<string, string | undefined>): string {
    ...
    const s = q.toString();
    return s ? `/contracts/payments?${s}` : '/contracts/payments';
  }

  return (
    <div className="px-6 lg:px-8 py-6 max-w-[1920px] mx-auto space-y-6 print:px-0 print:py-0">
      <Breadcrumbs
        items={[{ label: 'Contract Management', href: '/contracts/dashboard' }, { label: 'Payments' }]}
      />

      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-3xl font-semibold text-text-primary tracking-tight">Contract Payments</h1>
          <p className="mt-1.5 text-sm text-text-secondary">Track payment status across all contracts.</p>
        </div>
      ...
      <div className="hidden print:block">
        <h1 className="text-xl font-semibold">Contract Payments Register</h1>
        <p className="text-xs text-text-secondary">
          Generated {new Date().toLocaleString('en-GB')}
          {hasActiveFilters ? ' — filtered view' : ' — all payments'}
        </p>
      </div>
      ...
      <PaymentRegisterTable payments={payments} contracts={contracts} canUpdate={canUpdate} />
