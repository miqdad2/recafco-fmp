...
export async function GET(_request: Request, { params }: { params: Promise<{ taskId: string }> }): Promise<Response> {
  ...
  try {
    const comments = await contractsApi.listWorkflowTaskComments(taskId);
    return NextResponse.json({ data: comments });
  } catch (err) {
    return new NextResponse(err instanceof Error ? err.message : 'Failed to load comments', { status: 502 });
  }
}
