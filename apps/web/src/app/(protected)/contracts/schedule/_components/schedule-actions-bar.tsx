...
export function ScheduleActionsBar(): React.JSX.Element {
  const searchParams = useSearchParams();
  const qs = searchParams.toString();
  const exportHref = `/contracts/schedule/export${qs ? `?${qs}` : ''}`;
  ...
  return (
    <div className="flex items-center gap-2 print:hidden">
      <button
        ...
        title="Print the current filtered schedule (use your browser's Save as PDF option for a PDF copy)"
      >
      ...
      <a
        href={exportHref}
        className="inline-flex items-center gap-1.5 h-10 px-4 rounded-md border border-border bg-surface text-text-primary text-sm font-medium hover:bg-surface-secondary focus:outline-none focus:ring-2 focus:ring-focus"
        title="Export the current filtered schedule as CSV (opens in Excel)"
      >
